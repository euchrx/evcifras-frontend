...
export function LoginPage() {
  ...
  const [email, setEmail] = useState("admin@evcifras.com");
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md items-center">
      <form onSubmit={handleSubmit} className="w-full rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-2xl shadow-black/20">
        ...
        <h1 className="mt-6 text-3xl font-black text-white">Entrar no EvCifras</h1>
        <p className="mt-2 text-sm text-slate-400">Acesse suas cifras, favoritos e repertórios.</p>
        ...
        <p className="mt-5 text-center text-sm text-slate-400">
          Não tem conta?{" "}
          <Link to="/cadastro" className="font-semibold text-violet-300 hover:text-violet-200">
            Criar conta
          </Link>
        </p>
      </form>
